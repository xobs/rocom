...
esp_err_t wilma_cgi_sta_status_json(httpd_req_t *req)
{
	...
	{
		wilma_ip_info_t ip_info = wilma_get_ip_info();
		...
		esp_ip4addr_ntoa((const void *)&ip_info.ip, ip, IP4ADDR_STRLEN_MAX);
		esp_ip4addr_ntoa((const void *)&ip_info.gw, gw, IP4ADDR_STRLEN_MAX);
		esp_ip4addr_ntoa((const void *)&ip_info.netmask, netmask, IP4ADDR_STRLEN_MAX);
