...
#define WILMA_RETRIES_BEFORE_CONTINUING 2
...
static const char *NVS_NAMESPACE = "wilma";
...
wilma_ip_info_t wilma_get_ip_info(void)
{
	esp_netif_ip_info_t ip_info;
	wilma_ip_info_t wilma_ip_info;
	ESP_ERROR_CHECK(esp_netif_get_ip_info(ESP_NETIF_STA, &ip_info));
	wilma_ip_info.ip = ip_info.ip.addr;
	wilma_ip_info.netmask = ip_info.netmask.addr;
	wilma_ip_info.gw = ip_info.gw.addr;
	return wilma_ip_info;
}
	...
	if (event_base == WIFI_EVENT) {
		...
		case WIFI_EVENT_STA_DISCONNECTED: {
			...
			if (xEventGroupGetBits(WILMA_EVENT_GROUP) & WILMA_RETRY_CONNECTION_BIT) {
				...
				break;
			} else if (WILMA_RETRY_NUM < WILMA_RETRIES_BEFORE_CONTINUING) {
				WILMA_RETRY_NUM++;
				ESP_LOGD(TAG, "Trying again to connect to AP (try %d/%d)", WILMA_RETRY_NUM, WILMA_RETRIES_BEFORE_CONTINUING);
...
static void cfg_ap_fill_password(wifi_config_t *ap_config)
{
	...
	if (err != ESP_OK) {
		...
		memcpy(ap_config->ap.password, CONFIG_WILMA_AP_PASSWORD, strlen(CONFIG_WILMA_AP_PASSWORD));
		return;
	}
...
bool wilma_ap_ssid(char ssid[33])
{
	...
	err = nvs_get_str(WILMA_NVS_HANDLE, AP_PASSWORD_KEY, NULL, &password_length);
	if (err != ESP_OK) {
		password_length = strlen(CONFIG_WILMA_AP_PASSWORD);
	} else {
...
static void wifi_configure_softap(bool force_on)
{
	...
	esp_netif_ip_info_t ip_info;
	ip_info.ip.addr = esp_ip4addr_aton(CONFIG_WILMA_AP_IP);
	ip_info.gw.addr = esp_ip4addr_aton(CONFIG_WILMA_AP_GATEWAY);
	ip_info.netmask.addr = esp_ip4addr_aton(CONFIG_WILMA_AP_NETMASK);
	...
	wifi_config_t wifi_ap_config = {
		.ap =
			{
				.ssid = CONFIG_WILMA_AP_SSID_PREFIX,
				.ssid_len = strlen(CONFIG_WILMA_AP_SSID_PREFIX),
				.channel = CONFIG_WILMA_AP_CHANNEL,
				.password = CONFIG_WILMA_AP_PASSWORD,
				.max_connection = CONFIG_WILMA_AP_MAX_CONNECTIONS,
